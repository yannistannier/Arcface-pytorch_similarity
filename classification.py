...
def main_worker(gpu, ngpus_per_node, args):
    ...
    args.name = args.name+"_"+args.backbone+"-"+args.metric+"-"+args.loss+"-"+"x".join([str(x) for x in args.input_shape])
    args.name = args.name+"-f"+str(args.num_feature)+"-c"+str(args.num_classes) #+"-"+str(int(time.time()))

    save_path = os.path.join(args.checkpoints_path, args.name)
    ...
    load_pretrained_weights(model, torch.load(args.load_model_path))
    ...
    train_dataset = Dataset(args.train_root, args.train_list, phase='train', input_shape=args.input_shape)
    ...
    if args.val_root:
        # args.evaluate = True
        val_dataset = Dataset(args.val_root, args.val_list, phase='val', input_shape=args.input_shape)
...
def save_checkpoint(state, is_best, save_path, filename='checkpoint.pth.tar'):
    if is_best:
        torch.save(state, save_path+"/classif_best.pth.tar")
        # torch.save(state["state_dict"], save_path+"/classif_best.pth")
