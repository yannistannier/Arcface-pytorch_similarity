...
def validate(val_loader, model, criterion, device, opt):
...
if __name__ == '__main__':
    ...
    os.makedirs(os.path.join(save_path, "interval_classif"), exist_ok=True)
        ...
        for ii, data in enumerate(trainloader):
            ...
            if iters % opt.print_freq == 0:
                ...
                print('{} epoch {} it {} / {} {} it/s loss {} lr {} acc {}'.format(time_str, i, ii, total_iter,
                                                                                   ...
                                                                                   acc))
                with open(os.path.join(save_path, 'train_classif_log.txt'), 'a') as f:
                    f.write('{} epoch {} iter {} {} it/s loss {} lr {} acc {}\n'.format(time_str, i, ii,
                                                                                        np.around(speed, 2),
                                                                                        loss.item(), lr_t, acc))
                if opt.display:
                    writer.add_scalar('Loss_classif/train', loss.item(), iters)
                    writer.add_scalar('Acc_classif/train', acc, iters)

                start = time.time()

        scheduler.step()
        if i % opt.save_interval == 0 or i == opt.max_epoch:
            save_model(model, os.path.join(save_path, "interval_classif"), opt.backbone, i)

        if opt.val_list:
            val_acc1, val_loss = validate(valloader, model, criterion, device, opt)
            with open(os.path.join(save_path, 'val_classif_log.txt'), 'a') as f:
                f.write('Validation : loss : {} acc : {} \n'.format(val_loss, val_acc1))
            if opt.display:
                writer.add_scalar('Loss_classif/val', val_loss, i)
                writer.add_scalar('Acc_classif/val', val_acc1, i)
